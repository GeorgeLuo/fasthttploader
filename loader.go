...
func run(numClients int, messagesRing *fastclient.MessagesRing) {

	fmt.Println("calling loader.go: run")
...
func burstThroughput(cfg *loadConfig, messagesRing *fastclient.MessagesRing) {
	if messagesRing == nil {
		log.Fatalf("messagesRing is nil")
	}
	if messagesRing.NextMessage().Request == nil {
		log.Fatalf("messagesRing.NextMessage().Request is nil")
	}
...
func printState() {
	...
	// method generated by qtpl
	r.UpdateRequestDuration(client.RequestDuration())
	//r.BodySizeHistogram = client.BodySize()
	r.HasHeaders = append(r.HasHeaders, client.HasHeaders())
	r.Unlock()
}
